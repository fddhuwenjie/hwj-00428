...
export default function Contracts() {
  ...
  const canBatchSign = (c: Contract) => c.status === 'draft' || c.status === 'pending'
  const availableForBatch = filtered.filter(canBatchSign)

  const toggleSelect = (id: string) => {
    const contract = contracts.find(c => c.id === id)
    if (!contract || !canBatchSign(contract)) return
  ...
  const toggleSelectAll = () => {
    if (selectedIds.length === availableForBatch.length) {
      ...
      setSelectedIds(availableForBatch.map(c => c.id))
    }
  }
    ...
    <div className="p-8">
        ...
        <div className="flex items-center gap-3">
          {selectedIds.length > 0 && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-[#D4A843] text-white rounded-lg hover:bg-[#b8922e] transition-colors"
            >
              <Send size={18} />
              批量发起签署 ({selectedIds.length})
            </button>
          )}
          <button
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        {availableForBatch.length > 0 && (
          ...
          >
            {selectedIds.length === availableForBatch.length ? (
              <CheckSquare size={14} className="text-[#1E3A5F]" />
            ) : (
              <Square size={14} className="text-gray-400" />
            )}
            {selectedIds.length === availableForBatch.length ? '取消全选' : '全选可操作'}
          </button>
        )}
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-3 gap-5">
          {filtered.map((contract: Contract) => {
            ...
            const canSelect = canBatchSign(contract)

            return (
              <div
                key={contract.id}
                className={`bg-white rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow relative ${
                  isSelected ? 'ring-2 ring-[#D4A843]' : ''
                }`}
              >
                {canSelect && (
                  <button
                    onClick={(e) => { e.stopPropagation(); toggleSelect(contract.id) }}
                    className="absolute top-4 right-4"
                  >
                    {isSelected ? (
                      <CheckSquare size={18} className="text-[#D4A843]" />
                    ) : (
                      <Square size={18} className="text-gray-300 hover:text-[#D4A843]" />
                    )}
                  </button>
                )}
                ...
                <p className="text-xs text-gray-400 mb-1">版本：{contract.currentVersion}</p>
      ...
      {showBatchSign && (
        <BatchSignModal
          ...
          onSuccess={handleBatchSignSuccess}
        />
      )}
    </div>
  )
